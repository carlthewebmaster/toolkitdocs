...
-   [The CGI Module](ch_intro.html#ch_intro.intro_cgi)
...
Argument descriptions such as the expected number, type, position, mandatory and optional attributes are setup during an application’s initilization such as the application object’s ```Init()``` method (see [previous section](#application-framework)) by calling the ```CArgDescriptions``` class methods. Then, the arguments are extracted by calling the ```CArgs``` class methods.
...
The ```CNcbiApplication``` class which was [discussed earlier](#application-framework) provides a framework for writing portable applications. For writing portable multi-threaded applications, the CORELIB provides a ```CThreadedApp``` class derived from ```CNcbiApplication``` class which provides a framework for building multi-threaded applications.
...
To help with portability, integer types such as ```Int1```, ```Uint1```, ```Int2```, ```Uint2```, ```Int4```, ```Uint4```, ```Int8```, ```Uint8``` have been defined with constant limits. For example, a signed integer of two bytes size is defined as type ```Int2``` with a minimum size of ```kMin\_I2``` and a maximum size of ```kMax\_I2```. There are minimum and maximum limit constants defined for each of the different integer types.
...
[Figure 2](ch_intro.html#ch_intro.F2) shows the most common connection stream classes derived from ```CConn\_IOStream``` - ```CConn\_HttpStream```, ```CConn\_MemoryStream```, ```CConn\_ServiceStream```, and ```CConn\_SocketStream```.

![Figure 2. Connection stream classes](img/CConn_IOStream.gif)

Figure 2. Connection stream classes

```CConn\_HttpStream``` models a stream of data between and HTTP client and an HTTP server (such as a web server). The server end of the stream is identified by a URL of the form <span class="nctnt ncbi-monospace">http[s]://host[:port]/path[?query]```.
...
Important classes of HTML elements used in forms to input data are the input elements such as checkboxes, radio buttons, text fields, etc. The ```CHTML\_input``` class derived from the ```CHTML\_OpenElement``` class serves as the base class for a variety of input elements (see [Figure 5](ch_intro.html#ch_intro.F5)).
...
<span class="nctnt highlight">Note: ``````CRegexp``` has no replacement functionality - i.e. there is no API support for replacing matched text with provided text or matched groups.
...
-   [Cite this Page](#citation-window)
    ...
    <span class="bk_cite_avail">Available from: http://www.ncbi.nlm.nih.gov/toolkit/doc/book/ch\_intro```
...
-   [<span title="View results of testsuite runs on the automatic C++ Toolkit builds">Testsuite```](http://intranet.ncbi.nlm.nih.gov/ieb/ToolBox/STAT/test_stat/test_stat_ext.cgi)
